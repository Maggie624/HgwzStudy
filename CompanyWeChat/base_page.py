...
class BasePage:
    ...
    def click_by_js(self, by, locator, timeout=5):
        WebDriverWait(self.driver, timeout, 0.5, ignored_exceptions=TimeoutException)\
    ...
    def click_and_check(self, target, expect):
        def click_and_find(x):
            self.click_by_js(*target, timeout=2)
            length = self.find_elements_by_wait(*expect, timeout=2)
            return len(length) >= 1
        WebDriverWait(self.driver, 8, 0.5, ignored_exceptions=TimeoutException).until(click_and_find)
